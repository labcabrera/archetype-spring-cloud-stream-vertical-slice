...
Se pretende ser un punto de partida para microservicios que publican y consumen eventos, JPA para la persistencia, CQRS para la integración de commands y queries para equipos de desarrollo que no estén familiarizados con conceptos de arquitectura de software como DDD o event sourcing y se enfoquen en la implementación de la lógica de negocio.
...
El módulo _casestep_ define la lógica de un trámite. Un expediente podrá tener N trámites asociados, y este módulo consume eventos de creación de expedientes para iniciar los trámites correspondientes.

De cara a la persistencia, se ha desnormalizado el modelo para que cada módulo tenga su propia tabla en la base de datos, evitando joins entre tablas de diferentes módulos y sirva como ejemplo de boundary contexts independientes para la orquestación basada en eventos.
